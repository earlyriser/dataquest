...
for elem in nba_stats["pts"]:
    difference = elem - pts_mean
    square_difference = difference ** 2
...
def get_variance(my_data):
    ...
    for elem in my_data:
        ...
        square_difference = difference ** 2
        variance += square_difference
    return variance / len(my_data)

def get_dev(my_data):
    return get_variance(my_data)**(1/2)

mp_dev = get_dev(nba_stats["mp"])
ast_dev = get_dev(nba_stats["ast"])
...
d_10 = point_10 - mean
d_100 = point_100 - mean

point_10_std = d_10 /std_dev
point_100_std = d_100/std_dev
...
r_fta_pts, p_v = pearsonr(nba_stats["fta"], nba_stats["pts"])
r_stl_pf, p_v = pearsonr(nba_stats["stl"], nba_stats["pf"])
...
cov_stl_pf = get_covariance(nba_stats["stl"], nba_stats["pf"])
cov_fta_pts = get_covariance(nba_stats["fta"], nba_stats["pts"])
...
r1_fta_blk = cov(nba_stats["fta"], nba_stats["blk"])[0,1] / ((nba_stats["fta"].var() * nba_stats["blk"].var())** (1/2))